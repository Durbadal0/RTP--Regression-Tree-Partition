...
bart_fn_r=function(cov1,partitions,radius,b1,b2,N){
  
  partitions=partitions[1:N,]
  radius=radius[1:N,]
  p=length(partitions[1,])
  partitions1=matrix(partitions[order(partitions[,1],decreasing = FALSE),],ncol=p)
  partitions2=partitions1+radius
  ...
  for(i in 1:N){
    a=0
    for(j in 1:p){
      if(cov1[j] >= partitions1[i,j] & cov1[j] < partitions2[i,j]){a=a+1}
    }
  ...
  if(part_ind==0){
    jjj=b2
  }else{jjj=b1[part_ind]}
  
  return(jjj)
  
}

bart_fn_nim2 = nimbleFunction(run=function(cov1=double(1),partitions=double(2),radius=double(2),
                                           b1=double(1),b2=double(0),N=integer(0)){
  p <- length(partitions[1,])
  
  partitions2 <- partitions[1:N,]+radius[1:N,]
...
bart_fn_nim=nimbleRcall(function(cov1=double(1),partitions=double(2),radius=double(2),
                                 b1=double(1),b2=double(0),N=integer(0)){},Rfun='bart_fn_r',returnType=double(1))


Cbart_fn_nim=compileNimble(bart_fn_nim)
...
b2=99999
cov1=c(.39,.19)
bart_fn_r(cov1,partitions,radius,b1,b2,150)

bart_fn_nim(cov1,partitions,radius,b1,b2,N=150)
Cbart_fn_nim(cov1,partitions,radius,b1,b2,N=150)
bart_fn_nim2(cov1,partitions,radius,b1,b2,N=150)
...
bart_nimcode3 <- nimbleCode({
  ...
  for(k in 1:n) {
    ...
    predicted.y[k] <- bart_fn_nim2(cov2[k,1:p],parti[1:N3,1:p],radi[1:N3,1:p],b11[1:N3],b21,N2)
...
PART_MCMC_results=function(cov_mat,y2,N31=1000,n.iter =5000,
  ...
  results=list(RMSE=rmse,RMSE_lin=rmse_lin,y1=y2,pred.y=y.pred,N3=summary$mean[1])
...
library(tictoc)
tic()
job=PART_MCMC_results(cov_mat=cov2,y2=y1,n.iter=5000)
toc()

rlist::list.save(job,file="Part_job1_data1.rds")
job$RMSE
job$RMSE_lin
job$summary$mean[1]
